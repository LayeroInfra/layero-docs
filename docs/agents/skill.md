...
и является **источником правды**: инструкции MCP-сервера, `llms.txt`,
эти страницы и блок, который CLI пишет в `AGENTS.md`, сверяются с ним
гейтом. Если где-то написано иначе, чем в навыке, — ошибка там, а не в
навыке.
...
Если навык уже установлен в `.agents/skills/`, указатель просто ведёт на
него.
