...
Без входа работают `search_docs`, `check_copy` и `refactor_site` —
документация доступна всегда, сразу после подключения. Остальные
инструменты работают с аккаунтом Layero. Клиенты, поддерживающие OAuth
(Claude Code, Cursor, Codex), при первом вызове такого инструмента **сами
откроют браузер** — подтвердите вход, и подключение появится в клиенте
