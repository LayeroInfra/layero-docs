...
У проекта **один стабильный production-адрес** — apex `<org>-<project>.layero.ru`. Что именно отдаёт apex — указывает **production-pointer**, который двигается автоматически (push в default-ветку **или** прямая `layero deploy`-загрузка в проект без репозитория — она авто-промоутится в apex) либо вручную (промоут из UI/CLI). Именованные ветки (`--branch`) получают **временные preview-ссылки** на 24 часа и apex не трогают.
