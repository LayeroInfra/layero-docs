...
**Аккаунта ещё нет или человек отошёл от компьютера.**
`npx layero@latest deploy --claim --yes --json` публикует папку вообще без входа.
Сайт живёт час; в событии `claimable` приходит `claim_url` — агент передаёт
ссылку человеку, и только человек может забрать сайт в аккаунт. Режим
включается только флагом `--claim`: без него и без токена CLI просит войти
(`auth_required`). Так выкладываются
только статика и SPA: серверному приложению (SSR, фуллстек, контейнер) нужен
аккаунт — CLI откажет кодом `claim_static_only` ещё до выгрузки. Адрес у
такого сайта случайный, и поисковикам он закрыт.
...
`url` — живой публичный сайт (у проекта без репозитория загрузка публикуется
сразу). `ready` приходит, когда адрес уже отвечает сайтом (`edge_ready: true`):
CLI сам ждёт этого до 90 секунд. Его и показывайте пользователю.
`dashboard_url` — страница управления, не сайт (см.
[JSON-events схема](./json-events.md)).

### Когда детект ошибся

Событие `detected` — подсказка, а не решение: в проект CLI её не записывает,
фреймворк платформа определяет по загруженным файлам. Значения из
`layero.json` в ней учтены. `confident: false` значит «папку не узнали» — тогда
в событии есть `hint` и `next_action`: подпапка монорепо → `--root apps/web`,
фронт и бэк рядом → блоки `frontend` и `backend` в `layero.json`, свой скрипт
сборки → `"framework": "generic"`, сервер → `-t node_web`. План без выкатки
покажет `npx layero@latest deploy --dry-run`. Всё остальное — одним полем в
[`layero.json`](../deploys/layero-json.md): там таблица «симптом → что
