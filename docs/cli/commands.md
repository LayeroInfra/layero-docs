...
| `layero deploy --claim` | Деплой без аккаунта: временный сайт на час (только статика и SPA) и ссылка, по которой человек заберёт сайт. |
...
После привязки команда сама завершает мастер и запускает первую сборку.
Фреймворк, команду сборки и папку результата она в проект не записывает:
их определяет сборщик по репозиторию на каждой сборке, поэтому смена
фреймворка в репозитории подхватится сама, а `layero.json` их переопределит.
Записывается только решение: папка приложения в монорепо и менеджер пакетов
из `layero.json`.
