...
| `--claim` | Деплой без аккаунта: временный сайт на час и `claim_url`, по которому человек заберёт сайт. Только статика и SPA: серверное приложение (SSR, фуллстек, контейнер) отклоняется до выгрузки кодом `claim_static_only` (код выхода 4). Адрес случайный — имя папки и `--name` в него не попадают; поисковикам сайт закрыт (`robots.txt` с `Disallow: /` и `X-Robots-Tag: noindex, nofollow`). Только явным флагом: без `--claim` и без токена `deploy` просит войти (`auth_required`) — и в агентской среде тоже. Токен, код заявки и ссылка хранятся в `~/.layero/config.json`, а не в `.layero/project.json`: этот файл уходит в git. Забор сайта в аккаунт сбрасывает переменные окружения, хуки деплоя и настройки сборки, заведённые без аккаунта. Вместе с `--project` отклоняется кодом `claim_with_project` (код выхода 4). |
