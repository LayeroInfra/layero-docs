...
| `--claim` | Деплой без аккаунта: временный проект на 72 часа и `claim_url`, по которому человек заберёт сайт. Включается сам, когда нет токена, среда агентская (не терминал и не CI) и передан `--yes`. |
