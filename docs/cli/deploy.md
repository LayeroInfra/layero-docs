...
Локальный детект — быстрая проверка: `layero.json` он не читает и может уверенно ошибаться. Что платформа поняла на самом деле, видно по строкам `[config] …` в логе сборки.
