...
| `--claim` | Деплой без аккаунта: временный проект на 72 часа и `claim_url`, по которому человек заберёт сайт. Включается сам, когда нет токена, среда агентская (не терминал и не CI), передан `--yes` и проект **новый**: нет `--project`, а папка не привязана к проекту аккаунта. Существующий проект без токена — это вход (`auth_required`), не песочница. Вместе с `--project` отклоняется кодом `claim_with_project` (код выхода 4). |
