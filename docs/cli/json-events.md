...
Деплой без аккаунта (`layero deploy --claim`; только явным флагом). Платформа
завела временный проект и выдала токен на него; сайт живёт час, адрес у него
случайный, поисковикам он закрыт. Событие
...
| `project_id` | string | |
| `slug` | string | |
| `url` | string | адрес сайта — тот же, что в `ready`. Пока сайт временный, браузеру он отвечает переходом на `preview_url`; для проверок и агентов открыт как есть |
| `preview_url` | string | ссылка «для людей»: страница панели с сайтом в рамке и пометкой «временный сайт». Ею делятся |
...
Токен, код заявки и ссылка сохраняются в `~/.layero/config.json` (файл
доступен только вам); в `.layero/project.json` — только привязка папки: этот
файл уходит в git, а код забора в публичном репозитории отдал бы сайт первому
встречному (с CLI 0.11.8; код из `project.json` старых версий при следующей
выкатке переезжает в конфиг). Повторный `layero deploy` в той же папке обновляет
...
песочницы — вход не нужен. Забор сайта в аккаунт сбрасывает переменные
окружения, хуки деплоя и настройки сборки, заведённые без аккаунта: выложенные
файлы остаются, следующая сборка берёт настройки из файлов проекта.

Песочница создаёт только **новый** проект. Если передан `--project` или папка
привязана к проекту, заявки на который на этой машине нет, а токена нет, CLI
начинает вход: событие `auth_required` с `url` и `user_code`. Так же — без
`--claim` в новой папке: до 0.11.8 режим в агентской среде с `--yes`
включался сам, и папка уходила в открытый интернет без решения человека. Токен
песочницы из `~/.layero/config.json` идёт только в свой проект.

### `claim_status`

Итог `layero claim status [code]`. Без кода — заявка проекта папки из
`~/.layero/config.json` (у папок старых версий CLI — из `.layero/project.json`).
...
| `queued` | `waited_s` | Сборка ждёт свободный сборщик; печатается раз в 15 с до первого `stage`. Не ошибка — ждать дальше |
