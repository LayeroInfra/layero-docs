...
| `org` | string | slug организации |
| `database` | string | слаг или id базы |
| `request` | object | `method`, `path`, `as`, `user_id`, `query`, `schema` — что ушло в пробу; тело не повторяется |
...
| `headers` | object | заголовки ответа шлюза, которые передаёт платформа |
...
| `data_probe_method` | `layero data probe` с методом не из `GET`, `POST`, `PATCH`, `DELETE` либо `/whoami` не методом `GET` | `GET`, `POST`, `PATCH` или `DELETE`; `/whoami` — только `GET` |
| `data_probe_path` | В пути пробы есть `?`: параметры проба принимает только флагами `--query`. Запрос не отправлялся | Готовая команда с `--query` — в `next_action` |
| `data_probe_query` | `--query` не в виде `имя=значение` или одно имя указано дважды | `--query select=id,title --query price=gt.100`; несколько условий на одну колонку — одним параметром `or=(…)` |
| `data_probe_body` | Тело пробы не разобрано: не JSON, не объект и не массив, файл не читается, заданы и `--body`, и `--body-file`, или тело у `GET` и `DELETE` | Объект или массив JSON в `--body` либо `--body-file`, только для `POST` и `PATCH` |
...
| `data_probe_rejected` | Платформа отказала в пробе до запроса к шлюзу: путь не метод базы, у базы не включён вход, тело больше 64 КБ, больше 50 параметров, нет прав администратора. Причина — в `message`, запрос не выполнялся. Отказ самого шлюза (`401`, `403`) приходит не этим кодом, а событием `data_probe` | Исправить запрос по тексту отказа; пути методов — `layero data methods --db <база>` |
| `data_probe_not_rolled_back` | Проба записи прошла (ответ 200–399), а шлюз не подтвердил откат: данные могли измениться. Событие `data_probe` пришло перед ошибкой | Проверить данные базы; пробу записи не повторять, пока причина не найдена |
