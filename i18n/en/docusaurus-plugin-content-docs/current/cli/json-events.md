...
| `sandbox_claimed` | Another `layero deploy` in a sandbox folder, but the site has been taken into an account: the sandbox token is revoked. The token and the claim are forgotten, the folder stays linked — it is the same project | Sign in with the account that took the site (`npx layero@latest login`) and run the same `deploy` |
| `sandbox_expired` | Another `layero deploy` in a sandbox folder, but its time (1 hour) ran out and the sandbox was deleted. The token, the claim and the folder's link are removed | A new site without an account — `npx layero@latest deploy --claim`; for a site that lives longer than an hour, sign in and deploy into the account |
...
| `2` | sign-in needed | `auth_required`, `auth_expired`, `auth_timeout`, `sandbox_claimed` |
| `3` | not found | `sandbox_expired`, `project_unknown`, `project_not_found`, `org_unknown`, `database_unknown`, `env_not_found`, `domain_not_found`, `hook_not_found`, `connection_not_found`, `account_not_found`, `repo_not_found`, `claim_unknown`, `branch_without_env`, `no_deploy`, `no_deploys`, `no_runs`, `data_key_unknown` |
